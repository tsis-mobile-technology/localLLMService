...
class ClippableGLUParallelLinear(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # automatically.
        # Handle both standard and quantized parameters (weight, qweight, qzeros, scales) dynamically
        for param_name, param in self.linear.named_parameters():
            # Support both private _weight_loader attribute and property fallback
            orig_loader = getattr(param, "_weight_loader", None)
            loader_attr = "_weight_loader"
            
            if orig_loader is None and hasattr(param, "weight_loader"):
                orig_loader = param.weight_loader
                loader_attr = "weight_loader"

            if orig_loader is not None:
                def make_fused_loader(loader):
                    def _fused_weight_loader(p, loaded_weight, loaded_shard_id=None):
                        if loaded_shard_id is not None:
                            return loader(p, loaded_weight, loaded_shard_id)
                        half = loaded_weight.shape[0] // 2
                        loader(p, loaded_weight[:half], 0)
                        loader(p, loaded_weight[half:], 1)
                    return _fused_weight_loader

                # Set it back. If the property doesn't have a setter, write to private field '_weight_loader'
                if loader_attr == "_weight_loader":
                    param._weight_loader = make_fused_loader(orig_loader)
                else:
                    try:
                        param.weight_loader = make_fused_loader(orig_loader)
