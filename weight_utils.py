...
def _load_pt_file(bin_file: str) -> dict:
    """Load a PyTorch checkpoint file, handling legacy tar format and unpickling errors.

    PyTorch 2.6 changed the default of weights_only from False to True.
    Legacy tar format files and certain quantized checkpoints cannot be loaded with weights_only=True.
    This function tries weights_only=True first, then falls back to False
    for files from trusted sources (HuggingFace Hub).
    """
    try:
        return torch.load(bin_file, map_location="cpu", weights_only=True)
    except Exception as e:
        logger.warning(
            "Loading %s with weights_only=False (due to load failure: %s)",
